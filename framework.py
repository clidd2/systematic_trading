...
def breakout(df, st=50, lt=200):
    df['{}_dma'.format(st)] = df[r'Close/Last'].rolling(st).mean()
    df['{}_dma'.format(lt)] = df[r'Close/Last'].rolling(lt).mean()
...
def plot_breakout(df, st, lt):
    ...
    buy_and_hold = df['Close/Last'].apply(np.log).diff(1).cumsum().apply(np.exp)
...
def strategy_implement(df, func):
    ...
    strategy['daily_returns'] = strategy['Close/Last'].apply(np.log).diff(1)
...
def main():
    fpath = r'C:\Users\Owner\Desktop\programming\systematic_portfolio'
    fname = r'spx_futures.csv'
    ...
    df = pd.read_csv(os.path.join(fpath,fname), header=0, index_col=0).iloc[::-1]
